...
with col2:
    # Make URL a clickable link, breaking long lines
...
if st.button("🚀 Publish update", use_container_width=True):
    if not new_version or not new_url:
        ...
        st.balloons()
        # Wait a moment so balloons are seen, then refresh page
        time.sleep(1)
        st.rerun()
...
with footer_cols[1]:
    if logo_img:
        # Resize logo to small (30px height)
        small_logo = logo_img.copy()
        small_logo.thumbnail((30, 30))
        st.image(small_logo, width=30)
    st.markdown("DeZignArena — You Dream, We Design", unsafe_allow_html=False)